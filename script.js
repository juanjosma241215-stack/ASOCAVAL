...
// --- LÓGICA PARA EL ZOOM DE IMÁGENES (LIGHTBOX) ---

document.addEventListener("DOMContentLoaded", function() {
    // Obtenemos el visor y sus componentes
    const lightbox = document.getElementById("imageLightbox");
    const lightboxImg = document.getElementById("lightboxImage");
    const captionText = document.getElementById("lightboxCaption");
    const closeBtn = document.querySelector(".lightbox-close");

    // Seleccionamos la imagen que queremos que tenga zoom
    // En este caso, la imagen del BMC identificada por la clase .zoomable-image
    const bmcImage = document.querySelector(".zoomable-image");

    if (bmcImage) {
        // Al hacer clic en la imagen original
        bmcImage.addEventListener("click", function() {
            lightbox.style.display = "block"; // Mostramos el visor
            lightboxImg.src = this.src; // Copiamos la ruta de la imagen
            captionText.innerHTML = this.alt; // Copiamos el texto alternativo como subtítulo
            
            // Prevenimos el scroll del body mientras el visor está abierto
    ...
    // Función unificada para cerrar el visor
    ...
    // 2. Cerrar al hacer clic directamente en la imagen grande (cursor zoom-out)
    if (lightboxImg) {
        lightboxImg.addEventListener("click", closeLightbox);
    }

    // 3. Cerrar al hacer clic en el fondo oscuro (fuera de la imagen)
    lightbox.addEventListener("click", function(event) {
        // Aseguramos que el clic fue en el fondo y no en la imagen misma
        if (event.target === lightbox) {
            closeLightbox();
        }
    });

    // 4. Cerrar al presionar la tecla 'Esc'
